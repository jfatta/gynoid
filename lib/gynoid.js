...
const Events = require('./extension/socket-events');
...
module.exports = class Gynoid {
  constructor(name, adapter, config) {
    ...
    console.log('Connecting to the Events Server via WebSocket...');
    this.events.on('connect', () => {
      console.log('Connected to Events Server');
    });

    this.events.on('disconnect', () => {
      console.log('Disconnected from the Events Server');
    })
  }
  ...
  getExtension(extensionName, repository) {
    const extension = new Extension(this.adapter, extensionName, repository, this.config, this.events);
